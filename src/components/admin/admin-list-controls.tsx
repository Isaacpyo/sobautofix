...
export function AdminListFilters({ action, query, status, statusOptions, placeholder = "Search records…" }: { action: string; query: string; status: string; statusOptions: FilterOption[]; placeholder?: string }) {
  return (
    <form action={action} method="get" className="mt-5 flex flex-col gap-3 rounded-2xl border border-[#E4EAF0] bg-white p-4 sm:flex-row sm:items-end">
      ...
      {(query || status) && <Link href={action} className="grid min-h-11 place-items-center rounded-xl border border-[#D7E0E9] px-4 text-sm font-bold text-[#586575] hover:bg-[#F4F7FA]">Clear</Link>}
    </form>
  );
}

export function AdminPagination({ path, page, pageSize, totalItems, query, status }: { path: string; page: number; pageSize: number; totalItems: number; query: string; status: string }) {
