...
export function Footer({ settings = siteConfig }: { settings?: SiteSettings }) {
  ...
  return (
    <footer className="footer-premium relative overflow-hidden border-t border-[#1974E2]/20 bg-[#030712] pb-8 pt-14 text-[#C6D2DF] lg:pt-16">
      <Container>
        <div className="grid gap-x-8 gap-y-0 md:grid-cols-2 md:gap-x-12 md:gap-y-12 xl:gap-x-16 xl:grid-cols-[1.35fr_.95fr_.95fr_.9fr_1.1fr]">
          <div>
            <div className="w-32">
              <Logo inverse />
            </div>
            <p className="mt-5 max-w-sm text-xs leading-6">
              Professional diagnostics, fault finding, servicing, repairs and vehicle sales across Doncaster and South Yorkshire.
            </p>
            <div className="mt-6">
              <h2 className="text-xs font-bold text-white">Follow SOB Autofix</h2>
  ...
  return (
    <div>
      <details className="group border-b border-white/10 md:hidden">
        <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 text-sm font-bold text-white marker:content-none">
          {title}
          <ChevronDown size={18} className="shrink-0 text-[#67B9FF] transition-transform group-open:rotate-180" aria-hidden="true" />
        </summary>
        <nav aria-label={`${title} footer links`} className="grid gap-3 pb-5 pl-2 text-xs">
      ...
      <nav aria-label={`${title} footer links`} className="hidden md:block">
        <h2 className="text-sm font-bold text-white">{titleHref ? <Link href={titleHref} className="inline-flex items-center gap-2 hover:text-[#67B9FF]">{title}<ArrowRight size={15} /></Link> : title}</h2>
        <div className="mt-4 grid gap-2.5 text-xs"><FooterLinks links={links} action={action} /></div>
      </nav>
    </div>
  );
}
...
function FooterContact({ settings, contactLinks, directionsHref }: { settings: SiteSettings; contactLinks: ReturnType<typeof linksForSettings>; directionsHref: string }) {
  ...
  return <div><details className="group border-b border-white/10 md:hidden"><summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 text-sm font-bold text-white marker:content-none">Contact<ChevronDown size={18} className="shrink-0 text-[#67B9FF] transition-transform group-open:rotate-180" aria-hidden="true" /></summary><div className="pb-5">{content}</div></details><div className="hidden md:block"><h2 className="text-sm font-bold text-white">Contact</h2>{content}</div></div>;
}
...
function BusinessAddress({
  ...
  return (
    <address className={cnAddress("text-xs leading-5 not-italic", className)}>
