import { BadgePoundSterling, Banknote, CircleCheckBig, Clock3, FilePenLine, Plus, ReceiptText } from "lucide-react";
...
import { InvoiceExportControl } from "@/components/admin/invoice-export-control";
...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; date?: string; page?: string; notice?: string }> }) {
  ...
  return <>
    ...
    <section className="mt-8 grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-5" aria-label="Invoice summary">{metrics.map(({ label, value, icon: Icon, tone }) => <article key={label} className="rounded-2xl border border-[#E4EAF0] bg-white p-4 sm:p-5"><span className={`grid size-9 place-items-center rounded-xl ${tone}`}><Icon size={18} /></span><p className="mt-4 text-2xl font-extrabold text-[#071127] sm:text-3xl">{value}</p><p className="mt-1 text-xs font-bold text-[#667586] sm:text-sm">{label}</p></article>)}</section>
    ...
    <section className="mt-8"><div className="flex justify-between gap-4"><div><h2 className="text-2xl font-extrabold text-[#071127]">Invoice list</h2><p className="mt-1 text-sm text-[#667586]">Newest records appear first.</p></div><p className="text-sm font-bold text-[#667586]">{matchingCount} matching</p></div>
      <div className="mt-5 hidden overflow-x-auto rounded-2xl border border-[#E4EAF0] bg-white xl:block"><table className="w-full min-w-[1100px] table-fixed text-left text-sm"><colgroup><col className="w-[10%]" /><col className="w-[17%]" /><col className="w-[18%]" /><col className="w-[9%]" /><col className="w-[9%]" /><col className="w-[9%]" /><col className="w-[8%]" /><col className="w-[7%]" /><col className="w-[13%]" /></colgroup><thead className="bg-[#F4F7FA] text-xs font-extrabold tracking-wide text-[#586575] uppercase"><tr>{["Invoice", "Customer", "Vehicle", "Source", "Date", "Due", "Amount", "Status", "Actions"].map((heading) => <th key={heading} className={`px-4 py-4 ${heading === "Amount" ? "text-right" : ""}`}>{heading}</th>)}</tr></thead><tbody className="divide-y divide-[#E4EAF0]">{invoices.map((invoice) => <tr key={invoice.id} className="align-top hover:bg-[#F8FAFC]"><td className="px-4 py-4 font-mono font-black text-[#1974E2]">{invoice.invoice_number || "DRAFT"}</td><td className="px-4 py-4"><p className="font-extrabold text-[#071127]">{invoice.customer_name}</p><p className="mt-1 text-xs text-[#667586]">{invoice.customer_email || invoice.customer_phone || "No contact"}</p></td><td className="px-4 py-4">{vehicle(invoice)}</td><td className="px-4 py-4">{sourceLabel(invoice.source_type)}</td><td className="px-4 py-4">{formatDate(invoice.issue_date || invoice.created_at)}</td><td className="px-4 py-4">{formatDate(invoice.due_date)}</td><td className="px-4 py-4 text-right font-extrabold">{formatPence(invoice.total_pence)}</td><td className="px-4 py-4"><InvoiceStatusBadge status={invoice.status} /></td><td className="px-4 py-4"><Actions invoice={invoice} /></td></tr>)}</tbody></table></div>
      <div className="mt-5 grid gap-4 xl:hidden">{invoices.map((invoice) => <article key={invoice.id} className="rounded-2xl border border-[#E4EAF0] bg-white p-5"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-sm font-black text-[#1974E2]">{invoice.invoice_number || "DRAFT"}</p><h3 className="mt-2 text-xl font-extrabold text-[#071127]">{invoice.customer_name}</h3><p className="mt-1 text-sm text-[#667586]">{vehicle(invoice)}</p></div><InvoiceStatusBadge status={invoice.status} /></div><div className="mt-5 flex justify-between border-t border-[#E4EAF0] pt-4"><strong>{formatPence(invoice.total_pence)}</strong><Actions invoice={invoice} /></div></article>)}</div>
      {!invoices.length && <div className="mt-5 rounded-2xl border border-[#E4EAF0] bg-white p-10 text-center"><ReceiptText className="mx-auto text-[#1974E2]" /><h3 className="mt-4 text-xl font-extrabold text-[#071127]">No invoices found</h3><p className="mt-2 text-sm text-[#667586]">Create a manual invoice or start from an existing booking.</p></div>}
      <AdminPagination path="/admin/invoices" page={page} pageSize={invoiceDashboardPageSize} totalItems={matchingCount} totalPages={pages} query={query} status={status} additionalParams={{ date }} />
    </section>
  </>;
}
