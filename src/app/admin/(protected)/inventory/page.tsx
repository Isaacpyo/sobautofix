...
export default async function InventoryAdminPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; page?: string }> }) {
  ...
  return <>
    ...
    <div className="mt-5 max-h-[60vh] overflow-auto rounded-2xl border border-[#E4EAF0] bg-white">
      <table className="w-full min-w-[720px] text-left text-sm"><thead className="sticky top-0 z-10 bg-[#F4F7FA] text-xs uppercase text-[#667586] shadow-[0_1px_0_#E4EAF0]"><tr><th className="p-4">Vehicle</th><th className="p-4">Price</th><th className="p-4">Status</th><th className="p-4">Updated</th><th className="p-4"><span className="sr-only">Action</span></th></tr></thead><tbody>{vehicles.map((vehicle) => <tr key={vehicle.id} className="border-t border-[#E4EAF0] hover:bg-[#F8FAFC]"><td className="p-4"><span className="font-bold text-[#071127]">{vehicle.year} {vehicle.make} {vehicle.model}</span><span className="block text-xs text-[#667586]">{vehicle.registration || `/${vehicle.slug}`}</span></td><td className="p-4 font-bold">{formatCurrency(vehicle.price)}</td><td className="p-4 capitalize">{vehicle.status === "available" ? "Live" : vehicle.status}</td><td className="p-4 text-[#667586]">{new Date(vehicle.updated_at).toLocaleDateString("en-GB")}</td><td className="p-4 text-right"><AdminLoadingLink href={`/admin/inventory/${vehicle.id}`} className="inline-flex min-h-10 items-center rounded-lg border border-[#BCD6F6] px-3 text-xs font-extrabold text-[#1446A5] hover:bg-[#F1F7FF]" loadingTitle="Opening vehicle" loadingDescription="Please wait while the vehicle record opens.">View</AdminLoadingLink></td></tr>)}</tbody></table>
