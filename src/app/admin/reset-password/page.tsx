...
export default async function ResetPasswordPage() {
  ...
  if (!admin) redirect("/admin/login?error=invalid-link");
  if (requiresMfaChallenge(admin.assurance)) {
    redirect("/admin/mfa?returnTo=%2Fadmin%2Freset-password&stepUp=1");
  }
  if (!canChangeAdminPassword(admin.assurance)) redirect("/admin/login?error=invalid-link");
