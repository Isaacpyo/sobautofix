import { ArrowRight, BatteryCharging, Check, ImageIcon, MapPin, Smartphone, Wrench } from "lucide-react";
...
import { PremiumCta, ProcessFlow, SectionIntro, TechnicalStatement } from "@/components/marketing/experience";
...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <section className={`hero-grid relative overflow-hidden py-16 text-white sm:py-20 lg:py-28 ${styles.hero}`} data-motion="off">
        ...
        <Container className="relative grid items-center gap-12 lg:grid-cols-[1fr_.95fr]">
          <div className={styles.heroCopy}>
            <p className="mb-4 text-xs font-extrabold tracking-[.18em] text-[#67B9FF] uppercase">Automotive diagnostics / Doncaster</p>
      ...
      <section className="deferred-section py-20 sm:py-28">
        <Container>
          ...
          <div className={styles.servicesGrid}>
            <ServiceCard index={0} title="Advanced diagnostics" body="Scan relevant systems, analyse evidence and carry out targeted checks." href="/diagnostics" />
            <ServiceCard index={1} title="Auto electrical" body="Investigate wiring, sensors, power supplies, drains and communication faults." href="/diagnostics/electrical-fault-finding" icon="diagnostic" />
            <ServiceCard index={2} title="Repairs & servicing" body="Clear recommendations followed by approved maintenance and repair work." href="/services" icon="repair" />
            <ServiceCard index={3} title="Mobile & specialist" body="Practical mobile support, inspections, recovery requests and fleet work where suitable." href="/services/mobile-specialist" icon="inspection" />
          </div>
        </Container>
      </section>
      ...
      <section className="deferred-section diagnostic-panel py-20 text-white sm:py-28">
        <Container>
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div className={styles.diagnosticList}>{diagnostics.filter((item) => item.published).slice(0, 6).map((item, index) => <ServiceCard key={item.slug} index={index} title={item.name} body={item.summary} href={`/diagnostics/${item.slug}`} dark />)}</div>
          </div>
        </Container>
      </section>
      ...
      <TechnicalStatement eyebrow="Diagnosis philosophy" lineOne="Professional diagnostics." lineTwo="Not guesswork." body="A warning or fault code begins the investigation. Evidence-led testing helps reveal the root cause and supports a more informed repair decision." />
...
function Feature({ icon, title }: { icon: React.ReactNode; title: string }) {
  return <div className={styles.feature}><span className="text-[#168BFF]">{icon}</span><strong>{title}</strong></div>;
}
