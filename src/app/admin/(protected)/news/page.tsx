...
export default async function NewsAdminPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <>
    <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-xs font-extrabold tracking-widest text-[#1974E2] uppercase">Public publishing</p><h1 className="mt-2 text-4xl font-extrabold text-[#071127]">News &amp; Blog</h1><p className="mt-2 max-w-2xl text-sm text-[#667586]">Draft, schedule, preview and publish useful automotive articles.</p></div><Link href="/admin/news/new" className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#1974E2] px-4 font-bold text-white"><Plus size={17} />New article</Link></div>
