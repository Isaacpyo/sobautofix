...
export default async function TopLevelPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <PageHero eyebrow={slug === "about" ? undefined : content.eyebrow} title={content.title} body={content.body} cta={slug !== "about"} compact={slug === "about"} tightTop={slug === "about"} showTrustFacts={slug !== "about"}>{slug !== "about" ? <VehicleJourney compact source={slug} /> : undefined}</PageHero>
      <section className="py-20 sm:py-24"><Container className="grid gap-12 lg:grid-cols-[.85fr_1.15fr]"><div>{imageId && <ContextualServiceImage id={imageId} className="mb-8 min-h-96" />}<Eyebrow>Our approach</Eyebrow><h2 className="text-balance text-4xl font-extrabold text-[#071127] sm:text-5xl">{content.introHeading}</h2></div><div className="space-y-5 text-lg leading-8 text-[#586575]">{content.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></Container></section>
