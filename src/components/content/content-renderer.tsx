...
async function RenderSection({ section, slug, index }: { section: ContentSection; slug: string; index: number }) {
  switch (section.type) {
    ...
    case "serviceCards": { const items = [...services, ...diagnostics].filter((item) => section.slugs.includes(item.slug)); return <section className="py-20"><Container><h2 className="text-4xl font-extrabold text-[#071127]">{section.heading}</h2><div className="mt-8 grid gap-5 md:grid-cols-3">{items.map((item) => <ServiceCard key={item.slug} title={item.name} body={item.summary} href={`/${item.category === "diagnostics" ? "diagnostics" : "services"}/${item.slug}`} />)}</div></Container></section>; }
    case "process": return <section className="bg-[#071127] py-20 text-white"><Container><Eyebrow className="text-[#67B9FF]">The process</Eyebrow><h2 className="text-4xl font-extrabold">{section.heading}</h2><ol className="mt-8 grid gap-4 md:grid-cols-4">{section.steps.map((step, stepIndex) => <li key={step} className="rounded-2xl border border-white/10 bg-white/5 p-5"><span className="font-black text-[#67B9FF]">0{stepIndex + 1}</span><strong className="mt-7 block">{step}</strong></li>)}</ol></Container></section>;
    ...
    case "cta": return <section className="hero-grid py-16 text-center text-white"><Container><h2 className="text-4xl font-extrabold">{section.heading}</h2><p className="mx-auto mt-3 max-w-2xl text-[#C6D2DF]">{section.body}</p><ButtonLink className="mt-6" href={section.href}>{section.label}</ButtonLink></Container></section>;
