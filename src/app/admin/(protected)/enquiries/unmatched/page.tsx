...
export default async function UnmatchedInboundPage() {
  ...
  const [unmatchedResult, enquiriesResult] = client ? await Promise.all([
    client.from("unmatched_inbound_emails").select("id,sender_email,subject,text_body,reason,created_at").is("linked_enquiry_id", null).order("created_at", { ascending: false }).limit(100),
  ...
  return <>
    ...
    <div className="mt-8 grid gap-5">{unmatched.map((message) => <article key={message.id} className="rounded-2xl border border-amber-200 bg-white p-5 sm:p-6">
      ...
      {message.reason !== "automated_ignored" && enquiries.length > 0 && <form action={linkUnmatchedInboundAction} className="mt-4 flex flex-wrap items-end gap-3"><input type="hidden" name="unmatchedId" value={message.id} /><label className="grid flex-1 gap-1 text-sm font-bold text-[#071127]">Link to enquiry<select name="enquiryId" required className="min-h-11 rounded-xl border border-[#D7E0E9] bg-white px-3 font-normal"><option value="">Select an enquiry</option>{enquiries.map((enquiry) => <option key={enquiry.id} value={enquiry.id}>{enquiry.customers?.name || "Customer"} · {enquiry.type.replaceAll("_", " ")} · {new Date(enquiry.created_at).toLocaleDateString("en-GB")}</option>)}</select></label><button className="min-h-11 rounded-xl bg-[#071127] px-4 text-sm font-bold text-white">Link message</button></form>}
