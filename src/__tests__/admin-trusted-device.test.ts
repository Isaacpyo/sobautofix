...
import { findValidTrustedDevice, generateTrustedDeviceToken, hashTrustedDeviceToken, isSensitiveAdminPath, TRUSTED_DEVICE_COOKIE_OPTIONS, TRUSTED_DEVICE_LIFETIME_SECONDS } from "@/lib/auth/trusted-device";
...
async function recordFor(token: string, overrides: Record<string, unknown> = {}) {
  return { id: "device-id", user_id: "admin-id", token_hash: await hashTrustedDeviceToken(token), device_label: "Chrome on Windows", user_agent_summary: "Chrome on Windows", created_at: "2026-08-13T08:00:00.000Z", last_used_at: "2026-08-13T08:00:00.000Z", expires_at: "2026-08-20T08:00:00.000Z", revoked_at: null, ...overrides };
}
