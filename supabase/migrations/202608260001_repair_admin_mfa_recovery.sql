-- Repair migration for environments where 202608130008 is recorded as applied but
-- its policy, recovery tables, and RPCs are absent. This is intentionally forward-only.
-- The administrator already has verified TOTP, so mandatory MFA remains enabled.

create table if not exists public.admin_mfa_policy (
  singleton boolean primary key default true check (singleton),
  mandatory_mfa_enabled boolean not null default true,
...
create index if not exists admin_mfa_recovery_codes_active_idx
  on public.admin_mfa_recovery_codes (user_id, created_at)
  where used_at is null and revoked_at is null;

create table if not exists public.admin_mfa_recovery_sessions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
...
create index if not exists admin_mfa_recovery_sessions_active_idx
