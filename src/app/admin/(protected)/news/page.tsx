...
export default async function NewsAdminPage({ searchParams }: { searchParams: Promise<{ status?: string; page?: string }> }) {
  ...
  const articles = data || [];
  const totalPages = Math.max(1, Math.ceil(articles.length / ADMIN_LIST_PAGE_SIZE));
  const page = Math.min(requestedPage, totalPages);
  const visibleArticles = articles.slice((page - 1) * ADMIN_LIST_PAGE_SIZE, page * ADMIN_LIST_PAGE_SIZE);
  ...
  return <>
    ...
    <HighIntentArticleImportControl completed={importCompleted} />
