...
import { deleteContent, restoreContentRevision, saveContent } from "../../actions";
...
export default async function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data }, { data: revisions }, { data: mediaRows }] = await Promise.all([
    ...
    client.from("media_assets").select("id,alt_text,category,published").order("created_at", { ascending: false }),
  ]);
  if (!data) notFound();
  const entry = mapContentEntry(data as Parameters<typeof mapContentEntry>[0]);
  const media = (mediaRows || []).map((item) => ({ id: item.id, alt: item.alt_text, category: item.category || undefined, published: item.published }));
  return <>
    ...
    <ContentEditor articleMode media={media} entry={entry} action={saveContent} />
